...
de estado (los primeros 4.000 pasos mensuales son gratis) y lo que hay que orquestar son tres
o cuatro tareas en serie por pipeline. La máquina de estados usa `glue:startJobRun.sync`, que
espera a que cada job termine y falla la ejecución si el job falla: la misma semántica que una
dependencia en cualquier orquestador.
...
- Los jobs de Glue admiten una corrida a la vez, así que dos pipelines de fuente no pueden ir
  en paralelo: comparten `ingest_landing` y `silver_load`.
